...
function SellerWithdrawalsPage() {
  ...
  async function loadWithdrawals() {
    try {
      ...
      setError(loadError.response?.data?.message || "Không tải được lịch sử rút tiền của seller.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      />
      {loading ? <EmptyState title="Đang tải lịch sử rút tiền" description="Hệ thống đang đồng bộ dữ liệu rút tiền của seller." /> : null}
        ...
        <div className="space-y-6">
          <div className="grid gap-6 xl:grid-cols-[1.02fr_0.98fr]">
            <WithdrawalForm
              ...
              title="Yêu cầu rút tiền seller"
              description="Yêu cầu rút tiền sẽ dùng tài khoản thanh toán mặc định đang hoạt động của seller."
            ...
            <div className="space-y-4">
              <SummaryCard
                ...
                description="Số dư hiện có trong ví seller và sẽ bị giữ lại ngay khi tạo yêu cầu rút tiền."
              />
              <SummaryCard
                label="Khoảng rút cho phép"
                value={`${formatMoney(context?.min_amount)} - ${formatMoney(context?.max_amount)}`}
                description="Mức rút tối thiểu và tối đa đang áp dụng cho hệ thống."
              />
              <SummaryCard
          ...
          <DataTable
            columns={[
              ...
              { key: "amount", title: "Số tiền", render: (row) => <MoneyText value={row.amount} /> },
              {
                ...
                render: (row) => row.raw?.sellerPaymentAccount?.accountNumber || row.raw?.sellerPaymentAccount?.bankName || "--",
              },
            ...
            emptyDescription="Gửi yêu cầu mới để hệ thống bắt đầu xử lý payout cho seller."
          />
        </div>
      ) : null}
    </div>
