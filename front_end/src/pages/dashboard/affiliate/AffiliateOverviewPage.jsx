...
function getCompletedOrderTime(commission) {
  return new Date(
    commission?.raw?.walletCreditedAt ||
    commission?.raw?.fraudCheckedAt ||
    commission?.raw?.createdAt ||
    commission?.raw?.order?.createdAt ||
    0,
  ).getTime();
}
...
function AffiliateOverviewPage() {
  ...
  const recentCompletedOrders = [...commissions]
    .filter((item) => item.actual_amount > 0)
    .sort((a, b) => getCompletedOrderTime(b) - getCompletedOrderTime(a))
    .slice(0, 3);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        <MetricTile label="Link đã tạo" value={overview?.linkCount || 0} />
        <MetricTile label="Đơn đã hoàn tất" value={approvedCount} />
        <MetricTile label="Đơn chờ hoàn tất" value={pendingCount} />
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-xl font-semibold text-slate-900">Đơn đã hoàn tất gần đây</h3>
              <p className="mt-2 text-sm leading-7 text-slate-600">
                Ba đơn mới nhất đã hoàn tất và ghi nhận hoa hồng thực nhận cho affiliate.
              </p>
            </div>
          ...
          <div className="mt-5 space-y-4">
            {recentCompletedOrders.length ? (
              recentCompletedOrders.map((item) => (
              ...
              <EmptyState
                ...
                description="Các đơn đã hoàn tất gần đây sẽ hiển thị tại đây khi affiliate bắt đầu có hoa hồng thực nhận."
              />
            )}
          </div>
        </div>
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3 text-sm leading-7 text-slate-600">
            ...
            <div className="rounded-[1.5rem] bg-slate-50 p-4">Số dư khả dụng và yêu cầu rút tiền đang được đọc trực tiếp từ ví backend thay vì dữ liệu mô phỏng.</div>
